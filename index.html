...
<head>
<meta charset="UTF-8">
<title>Pneuma Tap Game Final</title>
<style>
  body {
    background: #111;
    color: #fff;
...
<div id="scene">
  <div id="ground"></div>
  <div id="car"></div>
  <div id="idle-warning">⚠ Idle</div>
  <div id="boost-warning"></div>
</div>

<div id="pressure">Давление: 0</div>

<div id="bar">
  <div id="fill"></div>
</div>

<button id="tap">Тапать 💨</button>
<button id="china">Китайские баллоны 🇨🇳 (1 раз)</button>

<script>
// ====== DATA ======
const cars = [
  { name:"Mercedes W220", img:"cars/w220.png", maxPressure:100, explosionChance:0.02 },
  { name:"Audi A8", img:"cars/a8.png", maxPressure:110, explosionChance:0.04 }
];
...
function explode(){
  alert("💥 Баллоны рванули!");
  fillEl.style.background="white";
  setTimeout(()=>updateUI(),200);
  if(saved){ alert("Китайские баллоны спасли 😬"); saved=false; pressure=Math.floor(currentCar.maxPressure*0.5);}
...
document.getElementById("tap").onclick=()=>{
  ...
  // проверка полного давления
  if(pressure >= currentCar.maxPressure){
    mainInflatedOnce = true;
    alert("✅ Машина полностью накачана!");
    round++; if(round >= cars.length){ alert("🏆 Весь люкс поднят!"); round=0;}
...
document.getElementById("china").onclick=()=>{
  if(!mainInflatedOnce){ alert("⚠ Сначала накачайте основные баллоны!"); return; }
  if(saved) return;
  saved=true; currentCar.explosionChance*=2;
  alert("Поставлены китайские баллоны 😬");
};
